...
<% if (Request.QueryString("rname") = "") then %>
        <form action="register.asp?mode=Register" id="FORM1" method="post" name="FORM1">
          <input name="Refer" type="hidden" value="<% =chkString(Request.ServerVariables("HTTP_REFERER"), "sqlstring") %>" />
          <input class="button" name="submit" type="submit" value="<%= txtAgree %>" />&nbsp;&nbsp;
        </form>
<% else %>
        <form action="register.asp?mode=Register&rname=<%= chkString(Request.Querystring("rname"),"sqlstring") %>" id="FORM1" method="post" name="FORM1">
